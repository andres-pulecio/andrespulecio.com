...
class Path {
    ...
    initPaths() {
        ...
        this.init('../models/tile.glb', 1, -1, 0.05, 9, 1, 0, 1, 0, 1);
        this.init('../models/tile.glb', 1, -1.2, 0.05, 15.2, 1, 0, 1, 0, 1);
        this.init('../models/tile.glb', 1, 1.8, 0.05, 11.3, 1, 0, 1, 0, 1);

        let zPosition, xPosition;
        for (let i = 0; i < 15; i += 1.2) {
            zPosition = 27;
            this.init('../models/tile.glb', 1, -2 + getRndInteger(1, 5), 0, zPosition + (i * 3), 1, 0, 1, 0, 1);
        }
        ...
        for (let i = 0; i < 30; i += 1.2) {
            zPosition = 72;
            xPosition = 2;
            this.init('../models/tile.glb', 1, xPosition + (i * 3), 0.05, zPosition - 2 + getRndInteger(1, 5), 1, 0, 1, 0, 1);
        }

        // Path center to information
        for (let i = 0; i < 35; i += 1.2) {
            zPosition = 74;
            this.init('../models/tile.glb', 1, -2 + getRndInteger(1, 5), 0.05, zPosition + (i * 3), 1, 0, 1, 0, 1);
        }
